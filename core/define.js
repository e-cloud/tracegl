...
function define(id, factory) {
    ...
    if (typeof process !== 'undefined') {
        ...
        module.exports = global.define = function (id, factory) {
            ...
            const req = function (m, id) {
                if (id instanceof Array || arguments.length != 2 || id.includes('!')) {
                ...
                if (global.define.log && f.includes('/')) process.stderr.write(`<[<[${f}]>]>`);
    ...
    innerDefine.main = document.body.getAttribute('define-main') || window.location.pathname.replace(/^(.*\/)(.*?)/, '$2').replace('.html', '');
    ...
    function script(file, parent, cb) {
        ...
        scriptElem.src = base + file + (file.includes('.js') ? '' : '.js') + reloadQuery;
        ...
        function load() {
            ...
            _factory.toString().replace(/require\s*\(\s*["']([^"']+)["']\s*\)/g, function (x, input) {
                if (input.charAt(0) != '.') {
                    return;
                }
