...
define(function (require, exports, module) {
    ...
    function init() {
        ...
        if (!gl) {
            ...
            function msg() {
                ...
                if (isChrome) {
                    ...
                    d += `${w}<br/><br/>To enable webGL in Safari (5.1 or higher) follow these 5 steps:<br/>1. Top left of the screen: click Safari then Preferences (opens window).<br/>2. Click the advanced tab (gear) in the preferences window, its the last tab on the right.<br/>3. At the bottom check the "Show Develop menu in menu bar" checkbox.<br/>   At the top of the screen between Bookmarks and Window a new Develop menu appears<br/>4. Click the Develop menu at the top, and select Enable WebGL (last item at the bottom)<br/>5. <a href="${location.href}">Click here to refresh the browser</a><br/>`;
        ...
        clip.style.left = clip.style.top = '-10px';
        clip.style.width = clip.style.height = '0px';
        clip.style.border = '0';
        ...
        window.onkeydown = function (e) {
            ...
            ki = setTimeout(function () {
                // only executed when press doesnt fire
                key(e, kc);
                gl.keydown();
            }, 0);
    ...
    gl.camera = function (cb) {
        ...
        window.navigator.webkitGetUserMedia({ audio: false, video: true }, function (s) {
    ...
    function font(f) {
        ...
        for (let i = 0, l = t.e - t.s; i < l; i++) {
            ...
            t.m[i] = t.c[i] + ia;
            t.t[i] = Math.floor((xp * gs - t.xp) / t.w * 256) << 16 | Math.floor(yp * gs / t.h * 256) << 24;
            if (i == 127 - t.s) {
                ...
                x.fillText(ch, xp * gs, yp * gs + px + (isGecko ? 0 : 1));
            }
        }
