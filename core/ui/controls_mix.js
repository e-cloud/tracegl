...
define(function (require, exports) {
    ...
    cm.resize = function (b) {
        ...
        b.m = function (n) {
            ...
            if (bx) {
                if (by) {
                    ui.cursor(bx == by ? 'nwse-resize' : 'nesw-resize');
                } else {
                    ui.cursor('ew-resize');
                }
            } else {
                ui.cursor(by ? 'ns-resize' : 'default');
            }
        };
    ...
    cm.edit = function (b, t, c, s, m) {
        ...
        b.u = function () {
            ...
            for (var r = p; r >= 0; r--) {
                if (t.t.charCodeAt(r) != 32) break;
            }
            p = p - r < q - p ? r : q;
            for (var e = p; e < t.t.length; e++) {
                if (t.t.charCodeAt(e) == 32) break;
            }
            for (var s = p; s >= 0; s--) {
                if (t.t.charCodeAt(s) == 32) break;
            }
            mark(s + 1, e);
            if (!ct) {
                ct = fn.dt();
            ...
            if (ui.key.s) {
                mark(cs, ce + v);
                    ...
                    mark(ce + v);
                        ...
                        mark(v > 0 ? fn.max(ce, cs) : fn.min(ce, cs));
                    }
        }
